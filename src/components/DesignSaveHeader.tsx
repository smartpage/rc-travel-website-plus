...
// Auto Open now comes from EditorOverlayContext; remove local hook
...
const DesignSaveHeader: React.FC = () => {
  ...
  const { enabled, autoOpen, setAutoOpen } = useEditorOverlay();
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', justifyContent: 'space-between' }} data-overlay-ui="1">
      {/* Left controls: Auto Open */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }} data-overlay-ui="1">
        <div
          ...
          style={{
            ...
            height: 20,
            borderRadius: 999,
            background: autoOpen ? '#16a34a' : '#374151',
            position: 'relative',
            cursor: 'pointer',
        ...
        <span style={{ fontSize: 11, color: '#a1a1aa' }}>Auto Open</span>
      </div>

      {/* Right controls: Save + Auto */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }} data-overlay-ui="1">
        ...
        <style>{`
          @keyframes ants {0%{background-position:0 0,0 100%,0 0,100% 0}100%{background-position:6px 0,-6px 100%,0 6px,100% -6px}}
        `}</style>
      </div>
    </div>
  );
